...
data = {
    'code': """
    select row_number() over(order by split(key,'_')[0] ) as `key`, split(key,'_')[0] AS `report_date_1583809351215`, split(key,'_')[1] AS `__shop_id___1584354954501`,cast(value as DECIMAL(15,2)) as `food_realamount_1584355710784` from (select compare(ARRAY(cast(report_date as string),cast(__shop_id__ as string)),food_realamount,'ym','0','sum') as m from `db_yqs_b_505`.`tbl_pos_bill_food` where 1=1) t LATERAL VIEW explode(t.m) tt as key ,value  order by report_date_1583809351215 ASC,__shop_id___1584354954501 ASC
   """
    ,
    'kind': "sql"
}
